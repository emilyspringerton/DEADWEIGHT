...
int main(int argc, char **argv) {
    const char *host = "127.0.0.1", *arch = "ripper", *name = NULL; int port = 7700, think = 0; long matches = 0; uint32_t seed = 1; int quiet = 1;
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--version")) { printf("dw_bot %s (rules: %d cards, hull %d)\n", DW_VERSION, num_cards(), start_hull()); return 0; }
            ...
            fprintf(stderr, "usage: dw_bot --archetype ripper|wall|mirror [--name N] [--host H] [--port P] [--matches N] [--think-ms N] [--seed S] [--verbose]\n"
