...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else { fprintf(stderr, "usage: dw_client [--host H] [--port P] [--name N] [--kind human|bot] [--auto random|first-legal|ripper|wall|mirror] [--matches N] [--seed S] [--think-ms N] [--iduna-url URL --guest-file F | --token JWT] [--quiet] [--version]\n"); return 2; }
    }
    ...
    if (iduna_url) {   /* guest account: register on first use (file holds player_id + guest_secret; lose it = account gone), else login */
        ...
        if (pid[0] && sec[0]) {
            if (dwi_guest_login(&idu, pid, sec, tok, sizeof tok) != 0) { fprintf(stderr, "dw_client: guest-login failed (account unrecoverable without its file)\n"); return 1; }
        } else {
            if (dwi_guest_register(&idu, name, pid, sizeof pid, sec, sizeof sec, tok, sizeof tok) != 0) { fprintf(stderr, "dw_client: guest-register failed\n"); return 1; }
    ...
    o.name = name; o.kind = kind; o.policy = policy; o.seed = seed; o.target_matches = matches; o.think_ms = think; o.verbose = !quiet; o.idle_timeout_ms = 60000; o.token = tok[0] ? tok : raw_token;
    int rc = dwc_run(&c, &o);
    dwc_close(&c);
