...
public final class MatchModel {
    ...
    void onRoundResult(Msg m) { hullYou = m.hullYou; hullOpp = m.hullOpp; log.add(new RoundLog(m)); }

    void onEnd(Msg m) { result = m.result; endReason = m.reason; }

    /** UI gate: slot -1 (pass) is always legal; slots 0-3 need a card and enough energy (PARENA is-legal-play). */
    public boolean canPlaySlot(int slot) {
        if (locked) return false;
        if (slot == -1) return true;
        if (slot < 0 || slot > 3) return false;
        return hand[slot] >= 0 && CardRules.isLegalPlay(hand[slot], energyYou);
    }
