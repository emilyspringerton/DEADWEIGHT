...
#   scripts/bot_pool.sh start [--host H] [--port P] [--think-ms N] [--bin PATH] [--pidfile F]
...
cd "$(dirname "$0")/.."
cmd="${1:-}"; shift || true
HOST=127.0.0.1; PORT=7700; THINK=300; BIN=build/dw_bot; PIDFILE=build/bot_pool.pids
while [ $# -gt 0 ]; do
  case "$1" in
    --host) HOST="$2"; shift 2;; --port) PORT="$2"; shift 2;; --think-ms) THINK="$2"; shift 2;;
    --bin) BIN="$2"; shift 2;; --pidfile) PIDFILE="$2"; shift 2;; *) echo "unknown arg $1" >&2; exit 2;;
  esac
done
case "$cmd" in
  start)
    ...
    for arch in ripper wall mirror; do
      "$BIN" --archetype "$arch" --name "bot-$arch" --host "$HOST" --port "$PORT" --think-ms "$THINK" --seed "$RANDOM" &
      echo $! >> "$PIDFILE"
    done
    echo "started 3 bots (ripper wall mirror) -> $HOST:$PORT, pids in $PIDFILE";;
  stop)
