...
W=240; H=336
KIND_NAMES=(burst tank shield)
KIND_FROM=('#E86A5F' '#6A9BF5' '#54C27A')   # top of gradient (light)
KIND_TO=('#6E1512' '#182F6B' '#144A28')     # bottom of gradient (dark)
...
glyph() { # $1 kind  $2 out.png  $3 fill  $4 cx  $5 cy  $6 size(W) $7 size(H)
  ...
  local pts
  case $k in
    burst) pts=$(awk -v cx=$cx -v cy=$cy 'BEGIN{n=16;for(i=0;i<n;i++){r=(i%2==0)?78:36;a=3.14159265*2*i/n-1.5708;printf "%d,%d ",cx+r*cos(a),cy+r*sin(a)}}')
           convert -size ${sw}x${sh} xc:none -fill "$fill" -draw "polygon $pts" "$out";;
    tank)  pts=$(awk -v cx=$cx -v cy=$cy 'BEGIN{for(i=0;i<6;i++){a=3.14159265*2*i/6-1.5708;printf "%d,%d ",cx+72*cos(a),cy+72*sin(a)}}')
    ...
    shield) local dx=$((cx-120)) dy=$((cy-150))
