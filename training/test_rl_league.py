...
class TestEloMath(unittest.TestCase):
    ...
    def test_expected_score_favors_the_higher_rated_player(self):
        self.assertGreater(elo_expected(1600, 1400), 0.5)
        self.assertLess(elo_expected(1400, 1600), 0.5)

    def test_expected_scores_are_complementary(self):
        ea = elo_expected(1550, 1450)
        eb = elo_expected(1450, 1550)
        self.assertAlmostEqual(ea + eb, 1.0)

    def test_win_raises_winner_and_lowers_loser(self):
        new_a, new_b = elo_update(1500, 1500, score_a=1.0)
        self.assertGreater(new_a, 1500)
        self.assertLess(new_b, 1500)

    def test_update_is_zero_sum(self):
        # Equal K on both sides -- what one gains, the other loses, exactly.
        new_a, new_b = elo_update(1500, 1600, score_a=1.0)
        self.assertAlmostEqual((new_a - 1500) + (new_b - 1600), 0.0)

    def test_draw_between_equals_changes_nothing(self):
        new_a, new_b = elo_update(1500, 1500, score_a=0.5)
    ...
    def test_underdog_win_gains_more_than_favorite_win(self):
        # An 1400 beating a 1600 should gain more Elo than a 1600 beating a 1400 would have.
        underdog_gain, _ = elo_update(1400, 1600, score_a=1.0)
        favorite_gain, _ = elo_update(1600, 1400, score_a=1.0)
        self.assertGreater(underdog_gain - 1400, favorite_gain - 1600)


class TestLeagueManagerElo(unittest.TestCase):
    def setUp(self):
    ...
    def test_unrated_member_defaults_to_default_elo(self):
        mgr = LeagueManager(self.tmpdir)
        self.assertEqual(mgr.get_elo("never-registered"), DEFAULT_ELO)

    def test_fresh_registration_starts_at_default_elo(self):
        mgr = LeagueManager(self.tmpdir)
        member = mgr.register(LeagueRole.MAIN, 0, "main0.zip")
        self.assertEqual(mgr.get_elo(member.id), DEFAULT_ELO)

    def test_new_generation_inherits_the_role_s_current_elo(self):
        mgr = LeagueManager(self.tmpdir)
        gen0 = mgr.register(LeagueRole.MAIN, 0, "main0.zip")
        mgr.set_elo(gen0.id, 1650.0)
        gen1 = mgr.register(LeagueRole.MAIN, 1, "main1.zip")
        self.assertEqual(mgr.get_elo(gen1.id), 1650.0,
                          "a fresh checkpoint of an already-proven role should inherit its rating, not reset to default")

    def test_explicit_reset_does_not_inherit(self):
        mgr = LeagueManager(self.tmpdir)
        gen0 = mgr.register(LeagueRole.MAIN_EXPLOITER, 0, "me0.zip")
        mgr.set_elo(gen0.id, 1800.0)
        reset_gen = mgr.register(LeagueRole.MAIN_EXPLOITER, 5, "me5.zip", inherit_elo_from_role=False)
        self.assertEqual(mgr.get_elo(reset_gen.id), DEFAULT_ELO,
                          "Main Exploiter's own real reset-to-a-fresh-network moment must not carry the old rating forward")

    def test_record_match_result_updates_both_sides_and_persists(self):
        mgr = LeagueManager(self.tmpdir)
        a = mgr.register(LeagueRole.MAIN, 0, "a.zip")
        b = mgr.register(LeagueRole.LEAGUE_EXPLOITER, 0, "b.zip")
        new_a, new_b = mgr.record_match_result(a.id, b.id, score_a=1.0)
        self.assertGreater(new_a, DEFAULT_ELO)
        self.assertLess(new_b, DEFAULT_ELO)
        # Re-read independently (a fresh LeagueManager instance) -- proves this is real, durable
        # persisted state, not just an in-memory echo of the call's own return value.
        mgr2 = LeagueManager(self.tmpdir)
        self.assertAlmostEqual(mgr2.get_elo(a.id), new_a)
        self.assertAlmostEqual(mgr2.get_elo(b.id), new_b)

    def test_three_archetypes_registered_per_snapshot_each_get_independent_elo(self):
        # Founder real-time: "each snapshot has the 3 archetypes the normal the exploiter and
        # the league exploiter so for each snapshot it adds 3 to the league" -- one generation's
        # worth of training produces THREE real, independently-tracked league members.
        mgr = LeagueManager(self.tmpdir)
        generation = 3
        members = [
            mgr.register(LeagueRole.MAIN, generation, "main3.zip"),
            mgr.register(LeagueRole.MAIN_EXPLOITER, generation, "me3.zip"),
            mgr.register(LeagueRole.LEAGUE_EXPLOITER, generation, "le3.zip"),
        ]
        self.assertEqual(len(mgr.all_members()), 3)
        self.assertEqual({m.generation for m in members}, {generation})
        self.assertEqual({m.role for m in members},
                          {LeagueRole.MAIN.value, LeagueRole.MAIN_EXPLOITER.value, LeagueRole.LEAGUE_EXPLOITER.value})
        # Each starts independently at the default -- registering three siblings together must
        # not accidentally cross-pollinate their Elo (they're different roles, not generations
        # of the SAME role).
        for m in members:
            self.assertEqual(mgr.get_elo(m.id), DEFAULT_ELO)


class TestRegisterGenerationSnapshot(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
    ...
    def _paths(self, gen):
        return {
            LeagueRole.MAIN: f"main{gen}.zip",
            LeagueRole.MAIN_EXPLOITER: f"me{gen}.zip",
            LeagueRole.LEAGUE_EXPLOITER: f"le{gen}.zip",
        }

    def test_registers_all_three_archetypes(self):
        registered = register_generation_snapshot(self.league, 0, self._paths(0))
        self.assertEqual(set(registered.keys()), set(ALL_ROLES))
        self.assertEqual(len(self.league.all_members()), 3)
        for role, member in registered.items():
            self.assertEqual(member.role, _role_str_for_test(role))
            self.assertEqual(member.generation, 0)

    def test_missing_archetype_raises_rather_than_partially_registering(self):
        incomplete = self._paths(0)
        del incomplete[LeagueRole.LEAGUE_EXPLOITER]
        with self.assertRaises(ValueError):
            register_generation_snapshot(self.league, 0, incomplete)
        # Real, deliberate all-or-nothing guarantee -- a caught bug must never leave a partial
        # snapshot sitting in the league.
        self.assertEqual(len(self.league.all_members()), 0)

    def test_successive_generations_accumulate_nine_members(self):
        register_generation_snapshot(self.league, 0, self._paths(0))
        register_generation_snapshot(self.league, 1, self._paths(1))
        register_generation_snapshot(self.league, 2, self._paths(2))
        self.assertEqual(len(self.league.all_members()), 9)
        self.assertEqual(len(self.league.members_by_role(LeagueRole.MAIN)), 3)

    def test_reset_roles_do_not_inherit_elo_but_others_do(self):
        gen0 = register_generation_snapshot(self.league, 0, self._paths(0))
        self.league.set_elo(gen0[LeagueRole.MAIN].id, 1700.0)
        self.league.set_elo(gen0[LeagueRole.MAIN_EXPLOITER].id, 1300.0)
        self.league.set_elo(gen0[LeagueRole.LEAGUE_EXPLOITER].id, 1550.0)

        gen1 = register_generation_snapshot(
            self.league, 1, self._paths(1), reset_roles={LeagueRole.MAIN_EXPLOITER})

        self.assertEqual(self.league.get_elo(gen1[LeagueRole.MAIN].id), 1700.0)
        self.assertEqual(self.league.get_elo(gen1[LeagueRole.LEAGUE_EXPLOITER].id), 1550.0)
        self.assertEqual(self.league.get_elo(gen1[LeagueRole.MAIN_EXPLOITER].id), DEFAULT_ELO,
                          "a reset role must start its new checkpoint back at the default rating")


def _role_str_for_test(role):
    return role.value if isinstance(role, LeagueRole) else role
