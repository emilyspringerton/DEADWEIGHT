...
void dw_policy_reset(DwPolicyCtx *c, uint32_t seed) {
    ...
    c->vault_you = start_vault(); c->lock_mask = 0;
    c->opp_kinds[0] = c->opp_kinds[1] = c->opp_kinds[2] = 3;
}

static int legal(const DwPolicyCtx *c, const int8_t hand[4], int i, int energy) {
    return hand[i] >= 0 && !((c->lock_mask >> i) & 1) && is_legal_play(hand[i], energy, c->vault_you);
}

/* best legal slot of `kind` by power (ties: lower slot); kind < 0 = any kind. -1 if none. */
static int best_of_kind(const DwPolicyCtx *c, const int8_t hand[4], int energy, int kind) {
    int best = -1;
    for (int i = 0; i < 4; i++)
        if (legal(c, hand, i, energy) && (kind < 0 || card_kind(hand[i]) == kind) && (best < 0 || card_power(hand[i]) > card_power(hand[best]))) best = i;
    return best;
}

static int interactive(const DwPolicyCtx *c, const int8_t hand[4], int energy, int round) {
    for (;;) {
        printf("round %d energy %d hand:", round, energy);
        for (int i = 0; i < 4; i++) printf(" [%d]=%d%s", i, hand[i], legal(c, hand, i, energy) ? "" : "x");
        printf("  play slot 0-3, or p to pass> "); fflush(stdout);
        char line[64];
        if (!fgets(line, sizeof line, stdin)) return -1;
        if (line[0] == 'p' || line[0] == 'P') return -1;
        if (line[0] >= '0' && line[0] <= '3' && legal(c, hand, line[0] - '0', energy)) return line[0] - '0';
...
int dw_policy_choose(int policy, DwPolicyCtx *c, const int8_t hand[4], int energy, int round) {
    int opts[5], n = 0, i;
    switch (policy) {
    case DW_POL_INTERACTIVE: return interactive(c, hand, energy, round);
    ...
    case DW_POL_RIPPER: {
        ...
        int b = best_of_kind(c, hand, energy, 0);
    ...
    case DW_POL_WALL: {
        ...
        return s >= 0 ? s : best_of_kind(c, hand, energy, 0);
    }
    case DW_POL_MIRROR: {
        ...
        if (c->last_opp_card >= 0) {
            ...
            for (int cand = 0; cand < 3; cand++) if (kind_beats(cand, k)) {
                int b = best_of_kind(c, hand, energy, cand);
                if (b >= 0) return b;
            }
        }
    ...
    default:
        opts[n++] = -1;
        for (i = 0; i < 4; i++) if (legal(c, hand, i, energy)) opts[n++] = i;
